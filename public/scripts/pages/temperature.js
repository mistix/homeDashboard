define(['jquery', 'tools'], function(jQuery, tools) {
    ...
    function drawChart() {
		jQuery('a[href="#last24h"]').click(function () {
			...
			jQuery.getJSON("/last_24_hours", function(data) {
		...
		jQuery('a[href="#last48h"]').click(function () {
			jQuery('#loading').addClass('modal');
			jQuery.getJSON("/last_48_hours", function(data) {
    ...
    function showTemperatureChart(data, chartTitle) {
		...
		var airTemperature = [];
		var expectedTemperature = 21.0;

		jQuery.each(data, function(index, value) {
			heaterTemperature.push(value.heaterTemperature);
			airTemperature.push(value.airTemperature);
			temperatureArray.push([tools.convertToDateTime(value.readingDateTime), value.airTemperature, value.heaterTemperature, expectedTemperature]);
		});
		...
		chartData.addColumn('number', 'Oczekiwana temperatura');
